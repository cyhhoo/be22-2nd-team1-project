...
public class AttendanceClosureController {
    ...
    public void close(@RequestBody AttendanceClosureRequest request) {
    ...
    public AttendanceClosureResponse findById(@PathVariable Long closureId) {
        // @PathVariable: URL 경로의 {closureId} 값을 Long으로 받아온다.
        // 단건 조회 로직은 QueryService에 위임하고 결과를 DTO로 반환한다.
        return attendanceClosureQueryService.findById(closureId);
    }

    @GetMapping // GET /api/attendance/closures
    public List<AttendanceClosureResponse> search(@ModelAttribute ClosureSearchRequest request) {
        ...
        return attendanceClosureQueryService.search(request);
    }
}
